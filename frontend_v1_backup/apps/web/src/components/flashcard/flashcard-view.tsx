...
export function FlashcardView() {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-3">
            ...
            <span>Thẻ Ghi Nhớ (SRS) - SuperMemo SM-2</span>
          </h2>
          <p className="text-xs font-medium text-slate-400 mt-1.5 flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold bg-orange-500/15 text-orange-300 border border-orange-500/25 px-2.5 py-0.5 rounded-full shadow-sm">
              {filters.lang === 'zh' ? '10,000 Thẻ Tiếng Trung' : '10,000 Thẻ Tiếng Anh'} (Bộ 20,000 Từ)
            </span>
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-slate-900/90 hover:bg-slate-800 border border-slate-800 text-slate-200 rounded-2xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 shadow-md"
          >
            <Upload className="w-3.5 h-3.5 text-orange-400" /> Nhập/Xuất Thẻ (CSV)
          </button>

          <div className="flex items-center gap-1 bg-slate-900/80 border border-slate-800 p-1.5 rounded-2xl backdrop-blur-xl shadow-md">
            <button
            ...
            >
              <Layers className="w-4 h-4 text-orange-400" /> Thẻ Lật Ôn Tập
            </button>
            ...
            >
              <BarChart3 className="w-4 h-4 text-orange-400" /> Thống Kê Ghi Nhớ
            </button>
          </div>
        </div>
      </div>

      {/* Language Workspace Switcher (Segmented Control) */}
      <div className="flex justify-center mb-2">
        <div className="bg-slate-900/80 p-1.5 rounded-2xl border border-slate-800 shadow-xl backdrop-blur-xl flex items-center gap-1">
          <button
            onClick={() => setFilters({ ...filters, lang: 'zh', level: '', topic: '', pos: '', status: 'all', specialMode: '' })}
            className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-black transition-all duration-300 cursor-pointer ${
              filters.lang === 'zh'
                ? 'bg-gradient-to-r from-red-600 to-rose-600 text-white shadow-lg shadow-red-500/30 border border-red-500/50'
      ...
      {/* Main Flashcard SRS Workspace */}
      {subTab === 'srs_deck' && (
        <div className="space-y-6">
          {/* 1. Flashcard Filter & Special Mode Toolbar */}
          <FlashcardFilterBar
            filters={filters}
          ...
          {/* 2. Interactive 3D Flashcard Deck Player */}
          {loading ? (
            <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-12 text-center space-y-3">
              <RefreshCw className="w-8 h-8 text-orange-500 animate-spin mx-auto" />
              <p className="text-xs text-slate-400 font-extrabold">Đang tải bộ thẻ ghi nhớ SRS...</p>
            </div>
          ) : cards.length === 0 ? (
            <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-10 text-center space-y-4 max-w-lg mx-auto">
              <BookOpen className="w-12 h-12 text-slate-600 mx-auto" />
              <div className="space-y-1">
                <h3 className="text-base font-extrabold text-white">Chưa có thẻ ghi nhớ nào phù hợp</h3>
                <p className="text-xs text-slate-400">
                  Thử thay đổi bộ lọc hoặc thêm thẻ mới từ trang Từ Điển Công Xưởng.
                </p>
              </div>
              <button
                onClick={() => setFilters({ lang: filters.lang, level: '', topic: '', pos: '', status: 'all', specialMode: '' })}
                className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-xl cursor-pointer transition-all"
              >
                Đặt Lại Bộ Lọc
              </button>
            </div>
      ...
      {/* SRS Analytics Dashboard Subtab */}
